...
class CirculateSwinAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.redeuce:
            ...
            #     x = self.linear(x)
        tensors.append(x)
        # last_tensor_np = tensors[-1].squeeze().detach().cpu().numpy()
        # last_tensor_np = (last_tensor_np - last_tensor_np.min()) / (last_tensor_np.max() - last_tensor_np.min())

        # plt.imshow(last_tensor_np, cmap='viridis')  # Adjust the colormap as needed
        # plt.axis('off')
        # plt.show()
        return x    
